    ...
    for counter, t in enumerate(targets):
        ...
        if not skip_default_queries:
            if not user_args.skip_defaults:
                ...
                ## emailrep seems to insta-block hhunt user agent without a key
...
def parse_args(args):
    ...
    parser = argparse.ArgumentParser(
        description="Email information and password lookup tool", prog="hhunt"
    )
    ...
    parser.add_argument(
        ...
        "-g",
        dest="gen_config",
        help="Generates a configuration file template in the current working directory & exits. Will overwrite existing hhunt_config.ini file",
        action="store_true",
        default=False,
    ),
