...
def print_banner(b_type="intro"):
	...
	"""
        ...
        print(
            c.fg.lightgrey,
            "\t\t  Official hhunt posts: \n\t\t  https://khast3x.club/tags/hhunt/\n\n",
            c.reset,
        )
...
def get_config_from_file(user_args):
    ...
    try:
        ...
        if user_args.cli_apikeys:
            if config.has_section("hhunt") is False:
                config.add_section("hhunt")
            ...
            for k in config["hhunt"]:
                if len((config["hhunt"][k])) != 0:
...
def check_latest_version():
    ...
    try:
        response = requests.request(
            url="https://api.github.com/repos/khast3x/hhunt/releases/latest", method="GET"
        )
        data = response.json()
        latest = data["tag_name"]
        if __version__ == data["tag_name"]:
            c.good_news("hhunt is up to date")
        else:
            c.bad_news(
                "Not running latest hhunt version. [Current: {current} | Latest: {latest}]".format(
...
def check_scylla_online():
    ...
    # https://github.com/khast3x/hhunt/issues/64
    try:
        re = requests.head(
