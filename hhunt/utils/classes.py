...
class target:
    ...
    def __init__(self, target_data, debug=False):
        self.headers = {
            "User-Agent": "hhunt-v.{h8ver}-OSINT-and-Education-Tool (PythonVersion={pyver}; Platform={platfrm})".format(
                h8ver=__version__,
    ...
    def get_intelx(self, api_keys):
        try:
            intel_files = []
            intelx = i(key=api_keys["intelx_key"], ua="hhunt-v.{h8ver}-OSINT-and-Education-Tool (PythonVersion={pyver}; Platform={platfrm})".format(
                h8ver=__version__,
                pyver=sys.version.split(" ")[0],
                platfrm=platform.platform().split("-")[0],
            ))
